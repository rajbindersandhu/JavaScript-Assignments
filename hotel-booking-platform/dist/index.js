...
import { signUpValidator, loginValidator, hotelSignUpValidator, roomSignUpValidator, bookingValidator, reviewValidator } from "./middleware/payload_validation.js";
import { signInAuth, verifyAuthToken } from "./middleware/auth.js";
import { insert_new_hotel, insert_new_user, insert_new_room, insert_new_booking, insert_new_review } from "./db/insert_db.js";
import { getUserByEmail, getRoomByNumber, getHotelById, getHotelJoinRoom, getAllRooms, getRoomById, getBookingByRoomId, getBookingByDates, getBookingByUserId, getBookingById, getReviewById, getReviewByBookid } from "./db/get_db.js";
import { updateBookingById, updateHotelWidRatNReview } from "./db/update_db.js";
...
//  })
app.post("/api/auth/signup", signUpValidator, async (req, res) => {
    const plainPass = req.body.password;
    const hashPassword = await hash(plainPass, 10);
...
app.post("/api/auth/login", loginValidator, signInAuth, async (req, res) => {
    const email = req.body.email;
    const db_res = await getUserByEmail(email);
    const token = generateToken(JSON.stringify({ "email": req.body.email, "password": req.body.password }));
    res.status(200).json({
        "success": true,
        "data": {
            "token": token,
...
app.post("/api/hotels", hotelSignUpValidator, verifyAuthToken("owner"), async (req, res) => {
    const hotel_id = `hotel_${crypto.randomUUID()}`;
    const db_payload = {
        "id": hotel_id,
...
app.post("/api/hotels/:hotelid/rooms", roomSignUpValidator, verifyAuthToken("owner"), async (req, res) => {
...
app.get("/api/hotels", verifyAuthToken("all"), async (req, res) => {
...
app.get("/api/hotels/:hotelId", verifyAuthToken("all"), async (req, res) => {
    const hotel_id = req.params.hotelId ?? "";
...
app.post("/api/bookings", bookingValidator, verifyAuthToken("customer"), async (req, res) => {
    ...
    if (!db_room_res || db_room_res.length == 0) {
        ...
        const bookingInsertPayload = {
            ...
            "roomId": room_id,
            "hotel_id": db_room_res[0].hotel_id,
...
app.get("/api/bookings", verifyAuthToken("customer"), async (req, res) => {
    const userId = res.get("customer_id") ?? "";
...
app.put("/api/bookings/:bookingId/cancel", verifyAuthToken("customer"), async (req, res) => {
    ...
    if (!db_res || db_res.length == 0) {
        ...
        const checkInDate = new Date(db_res[0].check_in_date);
...
app.post("/api/reviews", reviewValidator, verifyAuthToken("customer"), async (req, res) => {
    ...
    const checkoutDate = new Date(db_booking_res[0].check_out_date);
    const currentDate = new Date();
    ...
    const hotelRating = parseInt(db_hotel_res[0].ratings);
    const hotelTotalReview = parseInt(db_hotel_res[0].total_reviews);
