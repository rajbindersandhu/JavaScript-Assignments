...
async function signInAuth(req:Request, res: Response, next:NextFunction){
    ...
    const hashPass: string = dbResRows[0].password;
    const isSame: boolean = await validPassword(password, hashPass);
    if(isSame){
        next()
    }else{
...
function verifyAuthToken(role:string){
    return async (req: Request, res: Response, next: NextFunction) => {
        let data: string = "";
        try{
            const bearerStr: string = req.get("Authorization")!;
            if(!bearerStr.startsWith("Bearer ")){
                throw new Error('"Bearer ", not present in starting of string');
            }
            const authToken:string = bearerStr.replace("Bearer ", "") ?? "";
            console.log("JWT Token recived: ", authToken);
            data = <string>verifyToken(authToken);
        ...
        const userEmail =  (<User> JSON.parse(JSON.stringify(data))).email;
        ...
        if(!db_res || db_res.length == 0){
            ...
            const userRole: string = db_res[0].role;
